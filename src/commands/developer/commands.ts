...
export class CommandsCommand extends Subcommand {
            ...
            subcmd
              ...
              .addStringOption((option) =>
                option
                  .setName("guild")
                  .setDescription(
                    "Guild ID of the guild to list commands for (if not provided, will list global commands)"
                  )
                  .setRequired(false)
              )
          )
            ...
            subcmd
              ...
              .addStringOption((option) =>
                option
                  .setName("guild")
                  .setDescription(
                    "Guild ID of the guild to delete the command from (if not provided, will delete global command)"
                  )
                  .setRequired(false)
              )
          ),
  ...
  public async list(interaction: Subcommand.ChatInputCommandInteraction) {
    ...
    const embed = new EmbedBuilder().setTitle("Slash Commands");

    if (guildId) {
      embed.setDescription(
        `List of slash commands for guild ${interaction.options.getString(
          "guild"
        )}`
      );
    } else {
  ...
  public async delete(interaction: Subcommand.ChatInputCommandInteraction) {
    if (interaction.options.getString("guild")) {
      await this.container.client.guilds.cache
        .get(interaction.options.getString("guild")!)
        ?.commands.delete(interaction.options.getString("command")!);
      interaction.reply({
        content: `Deleted guild command \`${interaction.options.getString(
          "command"
        )}\` in guild ${interaction.options.getString("guild")}`,
      });
      console.log(
        `Deleted guild command ${interaction.options.getString(
          "command"
        )} in guild ${interaction.options.getString("guild")}`
      );
    } else {
      ...
      interaction.reply({
        content: `Deleted global command \`${interaction.options.getString(
          "command"
        )}\``,
      });
      console.log(
        `Deleted global command ${interaction.options.getString("command")}`
      );
    }
  }
}
